...
impl <F:PrimeField + ColEle + 'static, LK: LookupTableTwoCol<F> + Send + Sync + 'static> ComponentMapper<F,LK> for CpComponentMapper<F,LK>{
	...
	fn gen_nd_advice(&self, word: &Vec<F>, word_info: &WordInfo,
		...
		//1. expand to full length. F-level pad uses the canonical
		// pad stream starting at offset A (= within-F pad length
		// already emitted by pack_nibbles for the last real F).
