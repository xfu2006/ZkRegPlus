...
impl SedCapacity{
	...
	/// Inject the aggressive-mode forward-halo width into the FSM
	/// sub-capacity (rebuilds comp_capacities[1]). m=0 ⇒ no-op-equivalent.
	pub fn set_halo_nibbles(&mut self, m: usize){
...
impl Capacity for SedCapacityCombo{
	...
	fn halo_nibbles(&self) -> usize {
		self.cs.faa_capacity().halo_nibbles
	}
}
...
impl <F:PrimeField + ColEle,LK:LookupTableTwoCol<F>> SedComponentMapper<F,LK>{
	...
	pub fn new(
		mut cs_capacity: SedCapacity,
		mut igc_capacity: SedCapacity,
		...
		let m = clamdb.aggressive_max_span_nibbles;
		if m > 0 {
			assert!(cs_capacity.max_word_len * LEGS >= 4 * m,
				"halo M={} too large: need max_word_len*LEGS >= 4*M", m);
...
impl <F:PrimeField + ColEle + 'static, LK: LookupTableTwoCol<F> + Send + Sync + 'static> ComponentMapper<F,LK> for SedComponentMapper<F,LK>{
	...
	fn gen_nd_advice(&self, word: &Vec<F>, word_info: &WordInfo,
		...
		//truncate to M, pad the tail (last segment / short) with the
		//deterministic canonical stream. Empty when M==0.
		let m_halo = self.capacity.cs.faa_capacity().halo_nibbles;
		let halo_f: Vec<F> = if m_halo==0 { vec![] } else {
			let mut v: Vec<F> = word_info.halo_nibbles.iter()
				.take(m_halo).map(|b| F::from(*b as u32)).collect();
			if v.len() < m_halo {
				v.extend(utils::data::gen_pad_nibbles_fe::<F>(
					0, m_halo - v.len()));
			}
			v
		};
