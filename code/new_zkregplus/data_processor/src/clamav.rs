...
impl ClamavSig{
	...
	pub fn to_neg_automaton(&self, timeout_sec: usize)
		...
		log(LOG1, &format!("TO automaton{:?}: Attempt 1: single NFA", 
			self.name));
		log(LOG3, &format!("DETAILS: {:?}", self));
		let (sender, receiver) = mpsc::channel();
		let obj = self.clone();
		let _t = thread::spawn(move ||{
			let mut timer = Timer::new();
			...
			for item_id in 0..obj.eval_dnf.vec_disjunc.len(){
				...
				log(LOG3, &format!("--- Processing item: {:?}", &item));
				let mut item_nfa = empty_nfa();
				for i in 0..item.len(){
					let id = item[i];	
					let cur_nfa= build_nfa(&obj.vec_subsigs[id], obj.vec_bneg[id]);
					log(LOG3, &format!("---- ---- Handle id: {}, expr: {}, b_neg: {}, resulting dfa size: {:?}", id, obj.vec_subsigs[id], obj.vec_bneg[id], size_nfa(&cur_nfa)));
					item_nfa = if i==0 {cur_nfa.clone()} else {item_nfa.unite(cur_nfa)};
					//dfa = dfa.minimize_hop();
					log(LOG3, &format!("---- ----  UNITING NFA => {:?}", size_nfa(&item_nfa)));
				}
				log(LOG3, &format!("---- BEFORE NEGATE NFA size: {:?}", size_nfa(&item_nfa)));
				let item_nfa = item_nfa.negate();
				log(LOG3, &format!("---- AFTER NEGATE Item {:?} => NFA: {:?} \n", &item, size_nfa(&item_nfa)));
				nfa = if item_id==0 {item_nfa} else {nfa.unite(item_nfa)}; 
				log(LOG3, &format!("---- AFTER UNITE DNF automata => NFA: {:?} \n", size_nfa(&nfa)));
			}
			log_perf(LOG1, &format!("FINAL SIZE of NFA_easy: {}: {:?}", &obj.name, size_nfa(&nfa)), &mut timer);
		...
		log(LOG1, &format!("Attempt 2: Disjunction of Conjunction of DFA"));
		let (sender, receiver) = mpsc::channel();
		let obj = self.clone();
		let _t = thread::spawn(move ||{
			let mut timer = Timer::new();
			...
			for item_id in 0..obj.eval_dnf.vec_disjunc.len(){
				let mut vec_dfa = vec![];
				let item = &obj.eval_dnf.vec_disjunc[item_id];
				log(LOG1, &format!("--- Processing item: {:?}", &item));
				for i in 0..item.len(){
					let id = item[i];
					//note: already negated
					let cur_dfa= build_nfa(&obj.vec_subsigs[id], !obj.vec_bneg[id]);
					log(LOG1, &format!("---- ---- Handle id: {}, expr: {}, b_neg: {}, resulting negated dfa size: {:?}", id, obj.vec_subsigs[id], obj.vec_bneg[id], size_nfa(&cur_dfa)));
					vec_dfa.push(cur_dfa);
				}
				log(LOG1, &format!("--- vec_dfa size: {}", vec_dfa.len()));
				vec_res.push(vec_dfa);
			}
			log_perf(LOG1, &format!("FINAL SIZE of NFA_complex: {}: {:?}", &obj.name, get_total_size(&vec_res)), &mut timer);
			match sender.send(vec_res){
				Ok(()) => {},
				Err(_) => {}
			}
	...
	fn gen_eval_dnf(&mut self){
		log(LOG2, &format!("gen_eval_dnf: {}", self.expr));
		...
		log(LOG3, &format!("gen_eval_dnf RESULT: {:?}", self.eval_dnf));
	...
	fn preprocess_expr_new(&mut self, b_pm: bool){
		...
		log(LOG2, &format!("preprocess_expr_pm: name: {}, expr: {}", self.name, self.expr));
