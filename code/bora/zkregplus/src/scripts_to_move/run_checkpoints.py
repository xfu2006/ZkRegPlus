#!/usr/bin/env python3
"""
This file is generated by Claude Code
...
- Checkpoint cycle per tick:
    ...
    3. `criu dump` (single-dump, no pre-dump) streamed through criu-image-
       streamer | zstd into <slot>/final.tzst. The pre-dump optimization
       is intentionally skipped: this worker re-touches ~all RSS between
       checkpoints, so pre-dump only doubled raw bytes through zstd
       without shortening the freeze window.
    4. Atomically promote the slot by updating latest.txt.
    5. Delete rotated-out slots.

- Slots live at: <proj_root>/data/cache/checkpoints/slot_{a,b}/
  A pointer file latest.txt names the current slot. Each slot contains
  exactly one image file (final.tzst) plus meta.json.

- Resume mode: read latest.txt, stream-decompress that slot's final.tzst
  through criu-image-streamer in serve mode, hand it to `criu restore`.
  The Rust binary is NOT re-invoked -- the process image is resurrected
  with all in-memory state intact (including log handles,
  `initialized_jobs` HashSet, etc.).
...
def _build_user_env() -> dict:
    """Env for child process that restores the invoking user's PATH.

    sudo's `secure_path` strips ~/.cargo/bin, so a naive Popen here can't
    find `cargo` -- compile.sh exits with "cargo: command not found" and
    the worker pgrep loop times out. We look up SUDO_USER's home via the
    passwd db (NOT $HOME -- sudo points $HOME at /root) and prepend its
    .cargo/bin to PATH.
    """
    env = os.environ.copy()
...
def spawn_mode1(proj: Path) -> None:
    ...
    if shutil.which("cargo", path=env["PATH"]) is None:
        sys.exit(
            "ERROR: cargo not found on PATH for the spawned shell.\n"
            f"       Looked in: {env['PATH']}\n"
            "       (sudo strips PATH; SUDO_USER's ~/.cargo/bin should "
            "be auto-prepended -- check that the invoking user has "
            "rustup installed.)"
        )

    # Two-phase launch -- splits compile-visibility from worker stdio so
    # CRIU can dump/restore the worker without tripping its file-size
    # validation on fd 1/2.
    #
    #   Phase A (synchronous): cargo build via --no-run. Output streamed
    #   to /tmp/zkregplus_compile.log so the user can `tail -f`. This
    #   process is short-lived and exits before the worker exists, so
    #   CRIU never sees its FDs.
    #
    #   Phase B (detached): bash ./compile.sh -- second cargo invocation
    #   hits the cache, then exec's the test binary. Worker stdio routed
    #   to /dev/null, so the worker's fd 1 / fd 2 are device files, not
    #   regular files. CRIU handles those fine.
    #
    # Per-job Rust logs (data/cache/logs/log_job_*.txt) are written via
    # append_to_file inside the worker and stay on disk in their normal
    # location; they're already covered by the line-count rollback path.

    cargo_dir = proj / "zkregplus"
    compile_log_path = Path("/tmp/zkregplus_compile.log")

    print(f"Pre-compiling: cargo test --lib --release --no-run  "
          f"(cwd={cargo_dir})")
    print(f"  compile output -> {compile_log_path} "
          f"(tail -f to watch cargo progress)")
    with open(compile_log_path, "w") as compile_log_fh:
        rc = subprocess.run(
            ["cargo", "test", "--lib", "--release", "--no-run"],
            cwd=str(cargo_dir),
            stdin=subprocess.DEVNULL,
            stdout=compile_log_fh,
            stderr=subprocess.STDOUT,
            env=env,
        ).returncode
    if rc != 0:
        sys.exit(
            f"ERROR: cargo --no-run pre-compile failed (rc={rc}).\n"
            f"       See {compile_log_path} for build output."
        )
    print("  pre-compile OK")

    print(f"Spawning: bash ./compile.sh  (cwd={script_dir}, stdio=DEVNULL)")
    subprocess.Popen(
        ["bash", "./compile.sh"],
        cwd=str(script_dir),
        stdin=subprocess.DEVNULL,
...
def take_checkpoint(
    ...
    """Execute one single-dump checkpoint cycle.

    Returns a dict of timing and size stats. Caller is responsible for
    rotation and latest.txt updates.

    Sequence:
      1. Create <slot_dir> and a per-process socket dir for the streamer.
      2. Record current log-file line counts (for resume rollback).
      3. Spawn criu-image-streamer capture | zstd > <slot>/final.tzst
         and run `criu dump --stream` against it. Process is FROZEN for
         this phase only.
      4. Write <slot>/meta.json including log_line_counts.
    """
    ...
    freeze_sec = 0.0
    try:
        # --- Step 2: record log line counts -----------------------------
        # Done BEFORE the freeze. Any lines written between this record
        # and the actual freeze will be discarded on resume -- acceptable,
        # because the restored process memory image predates them and
        # will re-emit whatever logs it needs from its resumed state.
        log_line_counts = record_log_line_counts(logs_dir(proj))
        print(f"  logs:      recorded line counts for "
              f"{len(log_line_counts)} file(s)")

        # --- Step 3: streamed dump (process FROZEN) ---------------------
        ...
        # that terminates in slot/final.tzst. Single dump (no pre-dump):
        # for this workload the worker dirties most of memory each cycle,
        # so the pre-dump optimization only doubled raw bytes through
        # zstd without shortening freeze time.
        final_tzst = slot_dir / "final.tzst"
        freeze_start = time.time()
        _run_streamed_final_dump(
            pid, sock_dir, final_tzst, zstd_threads,
        )
        ...
        # Always clean up the socket dir, even on failure.
...
def _wait_for_streamer_sock(
    sock_dir: Path, streamer: subprocess.Popen, deadline: float,
) -> Path:
    """Block until criu-image-streamer creates its UNIX socket in sock_dir.

    The 1.0.0 binary names the socket 'streamer-capture.sock' or
    'streamer-serve.sock' depending on subcommand; we glob for any *.sock
    so version drift doesn't break us. Aborts if the streamer exits early
    or `deadline` (an absolute time.time() value) is reached.
    """
    while True:
        socks = list(sock_dir.glob("*.sock"))
        if socks:
            return socks[0]
        ...
        if time.time() > deadline:
            existing = sorted(p.name for p in sock_dir.iterdir())
            raise RuntimeError(
                f"timed out waiting for criu-image-streamer socket "
                f"in {sock_dir} (contents: {existing or 'empty'})"
            )
        time.sleep(0.05)
...
def _run_streamed_final_dump(
    ...
    with open(out_tzst, "wb") as out_fh:
        ...
        # criu-image-streamer 1.0.0 names the socket 'streamer-capture.sock'
        # (capture mode) -- glob for any *.sock so we don't break if the
        # name changes across versions.
        deadline = time.time() + 30
        sock_path = _wait_for_streamer_sock(sock_dir, streamer, deadline)
        ...
        _run_checked(
            [
                ...
                "-t", str(pid),
                "-D", str(sock_dir),
                "--stream",  # connect to streamer-capture.sock in -D
                "--leave-running",
                "--shell-job",
            ],
            "criu dump",
...
def restore_from_latest(proj: Path) -> int:
    ...
    Streams final.tzst back into a criu-image-streamer in "serve" mode,
    then invokes `criu restore`. The Rust binary is NOT re-invoked --
    its in-memory state is resurrected verbatim, which is why no
    b_resume flag is needed on this path.
    ...
    if not final_tzst.exists():
        sys.exit(f"ERROR: slot {slot} is missing final.tzst.")
    ...
    # --- Stream final.tzst into criu restore ------------------------------
    # Single-dump architecture: no pre.tzst, no --prev-images-dir.
    ...
    try:
        ...
        pid = _streamed_restore(final_tzst, sock_dir)
    finally:
        shutil.rmtree(sock_dir, ignore_errors=True)
...
def _streamed_restore(
    ...
    with open(final_tzst, "rb") as in_fh:
        ...
        # Wait for socket to be ready (serve mode names it
        # 'streamer-serve.sock' in 1.0.0; glob for robustness).
        deadline = time.time() + 30
        sock_path = _wait_for_streamer_sock(sock_dir, streamer, deadline)

        # Invoke criu restore. It reads from the streamer socket; no
        # --prev-images-dir because the slot has only one image file.
        _run_checked(
            [
                "criu", "restore",
                "-D", str(sock_dir),
                "--stream",  # connect to streamer-serve.sock in -D
                "--pidfile", str(pidfile),
                "--shell-job",
                "--restore-detached",
            ],
