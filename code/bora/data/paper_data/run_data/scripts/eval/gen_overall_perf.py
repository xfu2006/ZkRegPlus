...
For each dataset (Mal/Dna/Dlp) we read a FoldPot run log and emit one row:
  ...
  - Stage 1 (Main Folding), summed CPU-time over jobs, decomposed into
    ...
    and Total = Sel + Commit + Fold.  With balanced parallel jobs the
    wall-clock is approximately Total / Jobs.
...
def parse_log(text: str) -> dict:
    ...
    return {"corpus": corpus, "jobs": jobs, "steps": steps,
...
def build_table(per_dataset: dict) -> str:
    ...
    for ds, info in per_dataset.items():
        ...
        # Total CPU-time / wall-clock (= Total/Jobs under balanced jobs),
    ...
    return "\n".join([
        ...
        *rows,
        ...
        r"parallel jobs and the wall-clock time; for balanced jobs the wall-clock "
        r"is ${\approx}\,$Total$/$Jobs. \emph{Stage 2: Compression} computes "
