...
# ----------------------------------
"""Generate the dataset characterization table for §6.2 (tab:datasets).
...
(``\\textsc{Mal}`` etc.), the same identifiers used throughout §6 and
Appendix C.
...
Reads:
  ...
  row via ``common.extract_dlp_dataset`` (MS-DLP regex_zombie_international/
  for the rule count) with its corpus figures overridden by
  ``dlp_step1_corpus`` (the RE2-screened Enron listing in
  any_server/corpus.tgz) and its rule size by ``common.dlp_patkws_bytes``
  (pattern+keyword bytes from the jet1tb Zombie log, NOT the on-disk
  .regex files). All three rows are now live.
...
class Dataset:
    """One corpus x ruleset pairing. ``label`` is the §6/Appendix-C row stub."""
    ...
    rule_shape: str     # rule-shape description (LaTeX); NO LONGER emitted in
                        ...
                        # of the shape facts now stated in the §6.2 prose
...
def build_dna_dataset(d: dict) -> Dataset:
    ...
    return Dataset(
        label="Dna",
        # [t] top-aligns the two-line name; \shortstack centres it, which put
        # line 1 on the PREVIOUS row's baseline in the rendered PDF.
        corpus=(rf"\begin{{tabular}}[t]{{@{{}}l@{{}}}}"
                rf"chr{chrom}\\({assembly})\end{{tabular}}"),
        docs=1,
...
def build_dlp_dataset(d: dict) -> Dataset:
    ...
    return Dataset(
        label="Dlp",
        corpus=r"\begin{tabular}[t]{@{}l@{}}Enron\\Email\end{tabular}",
        docs=d["docs"],
...
def dna_drop_note(d: dict) -> str:
    ...
    Documents, in the generated table, where the §6.2 "65 dropped" footnote
