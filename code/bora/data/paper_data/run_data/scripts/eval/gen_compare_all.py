...
\begin{table*}[t]
  ...
  $u=""" + unit_str + r"""$~s\,B$^{-2}$ (Table~\ref{tab:zombie-data}). For
  ...
  ($""" + f"{rd['min_step']:.2f}" + r"""$\,s, the \texttt{proj\_512k} bucket):
  the \textsc{Dna} floor as that cost per signature, and Reef
  \textsc{Mal}/\textsc{Dlp} as that cost per signature per corpus byte
  (Table~\ref{tab:dna-reef-bora}).""" \
        + note_tex + r""" Cells shown as ``""" + PLACEHOLDER + r"""'' have no
