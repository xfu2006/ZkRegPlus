/* Modified 08/12/2024
   To adapt to FoldPot scheme where an additional
   cmF is added to committed instance.
   The circuit needs to verify that it corresponds to the
   first size-F elements of W.
*/
...
#[derive(Clone, Debug)]
pub struct DeciderEthCircuit<C1, GC1, C2, GC2, CS1, CS2, LK, const H: bool = false>
where
    C1: CurveGroup,
	...
	LK: LookupTableTwoCol<C1::ScalarField>,
{
    ...
    pub W_i: Option<WitnessFoldPot<C1>>,
    pub U_i1: Option<CommittedInstanceFoldPot<C1>>,
    pub W_i1: Option<WitnessFoldPot<C1>>,
    pub cmT: Option<C1>,
...
impl<C1, GC1, C2, GC2, CS1, CS2, LK, const H: bool> DeciderEthCircuit<C1, GC1, C2, GC2, CS1, CS2, LK, H>
where
    C1: CurveGroup,
    C2: CurveGroup,
    ...
    <C1 as CurveGroup>::BaseField: PrimeField,
{
    pub fn from_nova<
		...
		> (
        nova: FoldPot<C1, GC1, C2, GC2, FC, CS1, CS2, LK, H>,
    ) -> Result<Self, Error> {
        ...
        let (W_i1, U_i1) = NIFSFoldPot::<C1, CS1, H>::fold_instances(
		...
		let size_F = W_i1.size_F;
		let start_F = W_i1.start_F;

		//ADDED for comF
        let mut F = (&W_i1.W[start_F..start_F+size_F]).to_vec();
        F.extend(
            std::iter::repeat(C1::ScalarField::zero())
...
impl<C1, GC1, C2, GC2, CS1, CS2, LK> ConstraintSynthesizer<CF1<C1>>
    for DeciderEthCircuit<C1, GC1, C2, GC2, CS1, CS2, LK >
where
    C1: CurveGroup,
    C2: CurveGroup,
    GC1: CurveVar<C1, CF2<C1>>,
    ...
    for<'b> &'b GC2: GroupOpsBounds<'b, C2, GC2>,
{
    fn generate_constraints(self, cs: ConstraintSystemRef<CF1<C1>>) -> Result<(), SynthesisError> {
        ...
        let U_i1 = CommittedInstanceVarFoldPot::<C1>::new_input(cs.clone(), || {
            Ok(self.U_i1.unwrap_or(u_dummy_native.clone()))
        })?;
        let W_i1 = WitnessVarFoldPot::<C1>::new_witness(cs.clone(), || {
		...
		//9. Added check z_i is well-formed (and in-particular) its
		...
		zi_part2_inst_var.sum_lk_col2.enforce_equal(&eval_lkup_col2);

		/* TO IMPROVE: strangely using ark_r1cs_std::R1CS Var will
		cause SNARK set up throwing assignment missing error. Some
		conflicts between name space?  ONLY enable the following
		for debugging
		#[cfg(test)]{
    ...
    C::ScalarField: Absorb,
{
    pub fn get_challenges_native<T: Transcript<C::ScalarField>>(
        ...
        transcript.absorb_nonnative(&U_i.cmF);
    ...
    pub fn get_challenges_gadget<S: CryptographicSponge, T: TranscriptVar<CF1<C>, S>>(
        ...
        transcript.absorb(&U_i.cmF.to_constraint_field()?)?;
        let challenge_F = transcript.get_challenge()?;
...
pub mod tests {
    ...
    fn test_decider_circuit() {
		...
		type FC = SigmaIR1CS_Inst<Fr,Projective,Pedersen<Projective>,LK>;

        let mut rng = ark_std::test_rng();
        let poseidon_config = poseidon_canonical_config::<Fr>();
		let num_steps = 5;
		let (lkup, mut F_circuit, vec_stmt) = gen_six_root(num_steps);
		...
		let size_F = F_circuit.get_size_f();
        let prep_param = PreprocessorParamFoldPot::<
            Projective,
            Projective2,
			FC,
            Pedersen<Projective>,
			...
			LK,
            false,
        >::new(poseidon_config.clone(),F_circuit.clone(),lkup, size_F);
        ...
        let mut nova1 =
            FoldPot::<Projective, GVar, Projective2, GVar2, FC, CS1, CS2, LK, H>::init(
                &nova_params,
                F_circuit.clone(),
                z_0.clone(),
            )
        ...
        let mut nova =
            FoldPot::<Projective, GVar, Projective2, GVar2, FC,
				CS1, CS2, LK, H>::init(
                &nova_params,
                F_circuit.clone(),
                z_0.clone(),
		...
		for i in 0..num_steps{
			let v_stmt = vec_stmt[i].to_vec();
        	nova.prove_step(&mut rng, v_stmt, None).unwrap();
		}
    ...
    fn test_kzg_challenge_gadget() {
        ...
        let (challenge_W, challenge_E, challenge_F) =
        ...
        let (challenge_W_Var, challenge_E_Var, challenge_F_Var) =
        ...
        assert_eq!(challenge_F_Var.value().unwrap(), challenge_F);
    }
