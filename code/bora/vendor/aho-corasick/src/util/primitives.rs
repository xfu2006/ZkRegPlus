...
/// convert from pattern to state id
pub fn pattern_id_to_usize(s: PatternID)->usize{
	s.0.0.try_into().unwrap()
}

/// convert from usize to state id
pub fn state_id_from_usize(u: usize) -> StateID{
	StateID(SmallIndex(u.try_into().unwrap()))
}

/// convert from usize to pattern id
pub fn pattern_id_from_usize(u: usize) -> PatternID{
	PatternID(SmallIndex(u.try_into().unwrap()))
}
