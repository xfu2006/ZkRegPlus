...
impl <F: PrimeField + ColEle> GetSigAdvice<F>{
	...
	pub fn new(
		...
		if final_states.len()>olen{
			return Err(
				Error::CapErr(vec![(format!("olen: sigs, fsm_id: 0x{:x}", fsm_id), final_states.len()+1)])
			);
		}
		...
		if inp_sigs.len()>slen{
			return Err(
				Error::CapErr(vec![(format!("slen: sigs, fsm_id: 0x{:x}", fsm_id), inp_sigs.len())])
			);
		}
		...
		if final_states_sigs_count.len()>olen-1{
			return Err(
				Error::CapErr(vec![(format!("olen: sigs, fsm_id: 0x{:x}", fsm_id), 
		...
		if final_states_sigs.len()>jlen-1{
			return Err(
				Error::CapErr(vec![(format!("jlen: sigs, fsm_id: 0x{:x}", fsm_id), 
		...
		if sigs_to_merge.len()>slen-1{
			return Err(
				Error::CapErr(vec![(format!("slen: sigs, fsm_id: 0x{:x}", fsm_id), 
					sigs_to_merge.len()+1)])
			);
		}
