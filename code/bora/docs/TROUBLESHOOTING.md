...
Every `PAPER_DATA.py` run checks `/proc/sys/vm/max_map_count` against
1,073,741,824 and quits when it is lower and cannot be raised (a stock
kernel ships 65,530). Fix (free, no memory cost):
...
No root? `export ZKR_SKIP_MAP_COUNT_CHECK=1` bypasses the gate — safe
for `small`/`figs`, but a full run will hit the failure below.
...
`vm.max_map_count` ceiling: RAM looks free, the allocation still dies.
...
texlive-pictures`), or run `INSTALL.py --toolchain`. The generators
...
The default build uses the `light-test` feature. The commented full
dependency line in `crates/zkregplus/Cargo.toml` needs **~250 GB RAM**.
Do not switch it for reproduction — every menu item uses the shipped
configuration.

## `PREFLIGHT: NUMA split disabled; one unpinned process per leaf`

`numactl` is missing or `--preferred-many` unsupported. Not fatal: the
8-job leaves (`clam`, `dlp`) lose their 4+4 two-socket split and run as
one unpinned process — slower, same results.

## Run much slower than the quoted cost (laptop)

Proving is compute-bound; a *low-power* thermal profile roughly halves
the clock and doubles wall time. Switch to the performance profile.

## `RUNALL done: N generator(s) failed.` with N > 0

The `figs` PDF still builds — each failed generator's `figs/*.tex`
fragment keeps its previous (possibly stale) content. Scroll up to the
`[FAIL] <generator>.py` line for the cause, usually a missing raw-data
bundle for a leaf you have not run (`--data paper_data` restores the
recorded ones). Success is exactly `RUNALL done: 0 generator(s)
failed.`
